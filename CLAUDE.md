...
app/                    # Next.js App Router 前端
  page.tsx              # 首頁：推薦清單 + 同步/分析按鈕
  stocks/[symbol]/      # 個股頁：K 線圖 (lightweight-charts) + 法人籌碼
  api/                  # API routes
    sync/               # POST → SSE 串流，呼叫 ml/sync.py
    analyze/            # POST → 呼叫 rule_engine.py 或 predict.py
    recommendations/    # GET → 查詢 recommendations 表
...
| 資料 | 來源 | 單位 |
|---|---|---|
| 日 K 線 | TWSE/TPEX 個股月 API | 張（股÷1000）|
...
- K 線圖：lightweight-charts，upColor=#ef4444(紅), downColor=#22c55e(綠)
- sync_chips 分別追蹤 TSE/OTC 最新日期（避免 OTC 被跳過）
